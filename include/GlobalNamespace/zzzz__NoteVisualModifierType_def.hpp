#pragma once
// IWYU pragma private; include "GlobalNamespace/NoteVisualModifierType.hpp"
#include "beatsaber-hook/shared/types.hpp"
#include "../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include <cstddef>
#include <cstdint>
CORDL_MODULE_EXPORT(NoteVisualModifierType)
// Forward declare root types
namespace GlobalNamespace {
struct NoteVisualModifierType;
}
// Write type traits
MARK_VAL_T(::GlobalNamespace::NoteVisualModifierType);
DEFINE_IL2CPP_CLASS(::GlobalNamespace::NoteVisualModifierType, "", "NoteVisualModifierType");
// Dependencies
namespace GlobalNamespace {
// Is value type: true
// CS Name: NoteVisualModifierType
struct CORDL_TYPE NoteVisualModifierType {
public:
  // Declarations
  using __CORDL_BACKING_ENUM_TYPE = int32_t;

  /// @brief Nested struct __NoteVisualModifierType_Unwrapped
  enum struct __NoteVisualModifierType_Unwrapped : int32_t {
    __E_Normal = static_cast<int32_t>(0x0),
    __E_DisappearingArrow = static_cast<int32_t>(0x1),
    __E_Ghost = static_cast<int32_t>(0x2),
  };

  /// @brief Conversion into unwrapped enum value
  constexpr operator __NoteVisualModifierType_Unwrapped() const noexcept {
    return static_cast<__NoteVisualModifierType_Unwrapped>(this->value__);
  }

  /// @brief Conversion into unwrapped enum value
  constexpr explicit operator int32_t() const noexcept {
    return static_cast<int32_t>(this->value__);
  }

  // Ctor Parameters []
  // @brief default ctor
  constexpr NoteVisualModifierType();

  // Ctor Parameters [CppParam { name: "value__", ty: "int32_t", modifiers: "", def_value: None }]
  constexpr NoteVisualModifierType(int32_t value__) noexcept;

  /// @brief Field DisappearingArrow value: I32(1)
  static ::GlobalNamespace::NoteVisualModifierType const DisappearingArrow;

  /// @brief Field Ghost value: I32(2)
  static ::GlobalNamespace::NoteVisualModifierType const Ghost;

  /// @brief Field Normal value: I32(0)
  static ::GlobalNamespace::NoteVisualModifierType const Normal;

  /// @brief IL2CPP Metadata Type Index
  static constexpr uint32_t __IL2CPP_TYPE_DEFINITION_INDEX{ 5669 };

  /// @brief The size of the true value type
  static constexpr auto __IL2CPP_VALUE_TYPE_SIZE{ 0x4 };

  /// @brief Field value__, offset: 0x0, size: 0x4, def value: None
  int32_t value__;

  static constexpr bool __IL2CPP_IS_VALUE_TYPE = true;
};
// Non member Declarations
static_assert(offsetof(::GlobalNamespace::NoteVisualModifierType, value__) == 0x0, "Offset mismatch!");

static_assert(sizeof(::GlobalNamespace::NoteVisualModifierType) == 0x4, "Size mismatch!");

} // namespace GlobalNamespace
