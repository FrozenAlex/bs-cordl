#pragma once
// IWYU pragma private; include "Newtonsoft/Json/Utilities/AotHelper.hpp"
#include "beatsaber-hook/shared/types.hpp"
#include "../../../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include "System/zzzz__Object_def.hpp"
CORDL_MODULE_EXPORT(AotHelper)
namespace Newtonsoft::Json::Utilities {
template <typename T> class AotHelper___c__1_1;
}
namespace Newtonsoft::Json::Utilities {
template <typename T> class AotHelper___c__2_1;
}
namespace Newtonsoft::Json::Utilities {
template <typename TKey, typename TValue> class AotHelper___c__3_2;
}
namespace System {
class Action;
}
// Forward declare root types
namespace Newtonsoft::Json::Utilities {
class AotHelper;
}
namespace Newtonsoft::Json::Utilities {
template <typename T> class AotHelper___c__1_1;
}
namespace Newtonsoft::Json::Utilities {
template <typename T> class AotHelper___c__2_1;
}
namespace Newtonsoft::Json::Utilities {
template <typename TKey, typename TValue> class AotHelper___c__3_2;
}
// Write type traits
MARK_REF_T(::Newtonsoft::Json::Utilities::AotHelper*);
MARK_GEN_REF_T_PTR(::Newtonsoft::Json::Utilities::AotHelper___c__1_1);
MARK_GEN_REF_T_PTR(::Newtonsoft::Json::Utilities::AotHelper___c__2_1);
MARK_GEN_REF_T_PTR(::Newtonsoft::Json::Utilities::AotHelper___c__3_2);
DEFINE_IL2CPP_CLASS(::Newtonsoft::Json::Utilities::AotHelper*, "Newtonsoft.Json.Utilities", "AotHelper");
DEFINE_IL2CPP_GEN_CLASS_PTR(::Newtonsoft::Json::Utilities::AotHelper___c__1_1, "Newtonsoft.Json.Utilities", "AotHelper/<>c__1`1");
DEFINE_IL2CPP_GEN_CLASS_PTR(::Newtonsoft::Json::Utilities::AotHelper___c__2_1, "Newtonsoft.Json.Utilities", "AotHelper/<>c__2`1");
DEFINE_IL2CPP_GEN_CLASS_PTR(::Newtonsoft::Json::Utilities::AotHelper___c__3_2, "Newtonsoft.Json.Utilities", "AotHelper/<>c__3`2");
// Dependencies System.Object
namespace Newtonsoft::Json::Utilities {
// cpp template
template <typename T>
// Is value type: false
// CS Name: Newtonsoft.Json.Utilities.AotHelper/<>c__1`1<T>
class CORDL_TYPE AotHelper___c__1_1 : public ::System::Object {
public:
  // Declarations
  /// @brief Field <>9, offset 0xffffffff, size 0x8
  __declspec(property(get = getStaticF___9, put = setStaticF___9)) ::Newtonsoft::Json::Utilities::AotHelper___c__1_1<T>* __9;

  /// @brief Field <>9__1_0, offset 0xffffffff, size 0x8
  __declspec(property(get = getStaticF___9__1_0, put = setStaticF___9__1_0)) ::System::Action* __9__1_0;

  static inline ::Newtonsoft::Json::Utilities::AotHelper___c__1_1<T>* New_ctor();

  /// @brief Method <EnsureType>b__1_0, addr 0x0, size 0xffffffffffffffff, virtual false, abstract: false, final false
  inline void _EnsureType_b__1_0();

  /// @brief Method .ctor, addr 0x0, size 0xffffffffffffffff, virtual false, abstract: false, final false
  inline void _ctor();

  static inline ::Newtonsoft::Json::Utilities::AotHelper___c__1_1<T>* getStaticF___9();

  static inline ::System::Action* getStaticF___9__1_0();

  static inline void setStaticF___9(::Newtonsoft::Json::Utilities::AotHelper___c__1_1<T>* value);

  static inline void setStaticF___9__1_0(::System::Action* value);

protected:
  // Ctor Parameters []
  // @brief default ctor
  constexpr AotHelper___c__1_1();

public:
  // Ctor Parameters [CppParam { name: "", ty: "AotHelper___c__1_1", modifiers: "&&", def_value: None }]
  // @brief delete move ctor to prevent accidental deref moves
  AotHelper___c__1_1(AotHelper___c__1_1&&) = delete;

  // Ctor Parameters [CppParam { name: "", ty: "AotHelper___c__1_1", modifiers: "const&", def_value: None }]
  // @brief delete copy ctor to prevent accidental deref copies
  AotHelper___c__1_1(AotHelper___c__1_1 const&) = delete;

  /// @brief IL2CPP Metadata Type Index
  static constexpr uint32_t __IL2CPP_TYPE_DEFINITION_INDEX{ 13336 };

  static constexpr bool __IL2CPP_IS_VALUE_TYPE = false;
};
// Non member Declarations
} // namespace Newtonsoft::Json::Utilities
// Dependencies System.Object
namespace Newtonsoft::Json::Utilities {
// cpp template
template <typename T>
// Is value type: false
// CS Name: Newtonsoft.Json.Utilities.AotHelper/<>c__2`1<T>
class CORDL_TYPE AotHelper___c__2_1 : public ::System::Object {
public:
  // Declarations
  /// @brief Field <>9, offset 0xffffffff, size 0x8
  __declspec(property(get = getStaticF___9, put = setStaticF___9)) ::Newtonsoft::Json::Utilities::AotHelper___c__2_1<T>* __9;

  /// @brief Field <>9__2_0, offset 0xffffffff, size 0x8
  __declspec(property(get = getStaticF___9__2_0, put = setStaticF___9__2_0)) ::System::Action* __9__2_0;

  static inline ::Newtonsoft::Json::Utilities::AotHelper___c__2_1<T>* New_ctor();

  /// @brief Method <EnsureList>b__2_0, addr 0x0, size 0xffffffffffffffff, virtual false, abstract: false, final false
  inline void _EnsureList_b__2_0();

  /// @brief Method .ctor, addr 0x0, size 0xffffffffffffffff, virtual false, abstract: false, final false
  inline void _ctor();

  static inline ::Newtonsoft::Json::Utilities::AotHelper___c__2_1<T>* getStaticF___9();

  static inline ::System::Action* getStaticF___9__2_0();

  static inline void setStaticF___9(::Newtonsoft::Json::Utilities::AotHelper___c__2_1<T>* value);

  static inline void setStaticF___9__2_0(::System::Action* value);

protected:
  // Ctor Parameters []
  // @brief default ctor
  constexpr AotHelper___c__2_1();

public:
  // Ctor Parameters [CppParam { name: "", ty: "AotHelper___c__2_1", modifiers: "&&", def_value: None }]
  // @brief delete move ctor to prevent accidental deref moves
  AotHelper___c__2_1(AotHelper___c__2_1&&) = delete;

  // Ctor Parameters [CppParam { name: "", ty: "AotHelper___c__2_1", modifiers: "const&", def_value: None }]
  // @brief delete copy ctor to prevent accidental deref copies
  AotHelper___c__2_1(AotHelper___c__2_1 const&) = delete;

  /// @brief IL2CPP Metadata Type Index
  static constexpr uint32_t __IL2CPP_TYPE_DEFINITION_INDEX{ 13337 };

  static constexpr bool __IL2CPP_IS_VALUE_TYPE = false;
};
// Non member Declarations
} // namespace Newtonsoft::Json::Utilities
// Dependencies System.Object
namespace Newtonsoft::Json::Utilities {
// cpp template
template <typename TKey, typename TValue>
// Is value type: false
// CS Name: Newtonsoft.Json.Utilities.AotHelper/<>c__3`2<TKey,TValue>
class CORDL_TYPE AotHelper___c__3_2 : public ::System::Object {
public:
  // Declarations
  /// @brief Field <>9, offset 0xffffffff, size 0x8
  __declspec(property(get = getStaticF___9, put = setStaticF___9)) ::Newtonsoft::Json::Utilities::AotHelper___c__3_2<TKey, TValue>* __9;

  /// @brief Field <>9__3_0, offset 0xffffffff, size 0x8
  __declspec(property(get = getStaticF___9__3_0, put = setStaticF___9__3_0)) ::System::Action* __9__3_0;

  static inline ::Newtonsoft::Json::Utilities::AotHelper___c__3_2<TKey, TValue>* New_ctor();

  /// @brief Method <EnsureDictionary>b__3_0, addr 0x0, size 0xffffffffffffffff, virtual false, abstract: false, final false
  inline void _EnsureDictionary_b__3_0();

  /// @brief Method .ctor, addr 0x0, size 0xffffffffffffffff, virtual false, abstract: false, final false
  inline void _ctor();

  static inline ::Newtonsoft::Json::Utilities::AotHelper___c__3_2<TKey, TValue>* getStaticF___9();

  static inline ::System::Action* getStaticF___9__3_0();

  static inline void setStaticF___9(::Newtonsoft::Json::Utilities::AotHelper___c__3_2<TKey, TValue>* value);

  static inline void setStaticF___9__3_0(::System::Action* value);

protected:
  // Ctor Parameters []
  // @brief default ctor
  constexpr AotHelper___c__3_2();

public:
  // Ctor Parameters [CppParam { name: "", ty: "AotHelper___c__3_2", modifiers: "&&", def_value: None }]
  // @brief delete move ctor to prevent accidental deref moves
  AotHelper___c__3_2(AotHelper___c__3_2&&) = delete;

  // Ctor Parameters [CppParam { name: "", ty: "AotHelper___c__3_2", modifiers: "const&", def_value: None }]
  // @brief delete copy ctor to prevent accidental deref copies
  AotHelper___c__3_2(AotHelper___c__3_2 const&) = delete;

  /// @brief IL2CPP Metadata Type Index
  static constexpr uint32_t __IL2CPP_TYPE_DEFINITION_INDEX{ 13338 };

  static constexpr bool __IL2CPP_IS_VALUE_TYPE = false;
};
// Non member Declarations
} // namespace Newtonsoft::Json::Utilities
// Dependencies System.Object
namespace Newtonsoft::Json::Utilities {
// Is value type: false
// CS Name: Newtonsoft.Json.Utilities.AotHelper
class CORDL_TYPE AotHelper : public ::System::Object {
public:
  // Declarations
  template <typename T> using __c__1_1 = ::Newtonsoft::Json::Utilities::AotHelper___c__1_1<T>;

  template <typename T> using __c__2_1 = ::Newtonsoft::Json::Utilities::AotHelper___c__2_1<T>;

  template <typename TKey, typename TValue> using __c__3_2 = ::Newtonsoft::Json::Utilities::AotHelper___c__3_2<TKey, TValue>;

  /// @brief Field s_alwaysFalse, offset 0xffffffff, size 0x1
  __declspec(property(get = getStaticF_s_alwaysFalse, put = setStaticF_s_alwaysFalse)) bool s_alwaysFalse;

  /// @brief Method Ensure, addr 0x5d15c18, size 0x17c, virtual false, abstract: false, final false
  static inline void Ensure(::System::Action* action);

  /// @brief Method EnsureDictionary, addr 0x0, size 0xffffffffffffffff, virtual false, abstract: false, final false
  template <typename TKey, typename TValue> static inline void EnsureDictionary();

  /// @brief Method EnsureList, addr 0x0, size 0xffffffffffffffff, virtual false, abstract: false, final false
  template <typename T> static inline void EnsureList();

  /// @brief Method EnsureType, addr 0x0, size 0xffffffffffffffff, virtual false, abstract: false, final false
  template <typename T> static inline void EnsureType();

  /// @brief Method IsFalse, addr 0x5d15d94, size 0x5c, virtual false, abstract: false, final false
  static inline bool IsFalse();

  static inline bool getStaticF_s_alwaysFalse();

  static inline void setStaticF_s_alwaysFalse(bool value);

protected:
  // Ctor Parameters []
  // @brief default ctor
  constexpr AotHelper();

public:
  // Ctor Parameters [CppParam { name: "", ty: "AotHelper", modifiers: "&&", def_value: None }]
  // @brief delete move ctor to prevent accidental deref moves
  AotHelper(AotHelper&&) = delete;

  // Ctor Parameters [CppParam { name: "", ty: "AotHelper", modifiers: "const&", def_value: None }]
  // @brief delete copy ctor to prevent accidental deref copies
  AotHelper(AotHelper const&) = delete;

  /// @brief IL2CPP Metadata Type Index
  static constexpr uint32_t __IL2CPP_TYPE_DEFINITION_INDEX{ 13339 };

  static constexpr bool __IL2CPP_IS_VALUE_TYPE = false;
};
// Non member Declarations
static_assert(sizeof(::Newtonsoft::Json::Utilities::AotHelper) == 0x10, "Size mismatch!");

} // namespace Newtonsoft::Json::Utilities
