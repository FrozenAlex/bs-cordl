#pragma once
// IWYU pragma private; include "OVR/OpenVR/EVRFirmwareError.hpp"
#include "beatsaber-hook/shared/types.hpp"
#include "../../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include <cstddef>
#include <cstdint>
CORDL_MODULE_EXPORT(EVRFirmwareError)
// Forward declare root types
namespace OVR::OpenVR {
struct EVRFirmwareError;
}
// Write type traits
MARK_VAL_T(::OVR::OpenVR::EVRFirmwareError);
DEFINE_IL2CPP_CLASS(::OVR::OpenVR::EVRFirmwareError, "OVR.OpenVR", "EVRFirmwareError");
// Dependencies
namespace OVR::OpenVR {
// Is value type: true
// CS Name: OVR.OpenVR.EVRFirmwareError
struct CORDL_TYPE EVRFirmwareError {
public:
  // Declarations
  using __CORDL_BACKING_ENUM_TYPE = int32_t;

  /// @brief Nested struct __EVRFirmwareError_Unwrapped
  enum struct __EVRFirmwareError_Unwrapped : int32_t {
    __E_None = static_cast<int32_t>(0x0),
    __E_Success = static_cast<int32_t>(0x1),
    __E_Fail = static_cast<int32_t>(0x2),
  };

  /// @brief Conversion into unwrapped enum value
  constexpr operator __EVRFirmwareError_Unwrapped() const noexcept {
    return static_cast<__EVRFirmwareError_Unwrapped>(this->value__);
  }

  /// @brief Conversion into unwrapped enum value
  constexpr explicit operator int32_t() const noexcept {
    return static_cast<int32_t>(this->value__);
  }

  // Ctor Parameters []
  // @brief default ctor
  constexpr EVRFirmwareError();

  // Ctor Parameters [CppParam { name: "value__", ty: "int32_t", modifiers: "", def_value: None }]
  constexpr EVRFirmwareError(int32_t value__) noexcept;

  /// @brief Field Fail value: I32(2)
  static ::OVR::OpenVR::EVRFirmwareError const Fail;

  /// @brief Field None value: I32(0)
  static ::OVR::OpenVR::EVRFirmwareError const None;

  /// @brief Field Success value: I32(1)
  static ::OVR::OpenVR::EVRFirmwareError const Success;

  /// @brief IL2CPP Metadata Type Index
  static constexpr uint32_t __IL2CPP_TYPE_DEFINITION_INDEX{ 8414 };

  /// @brief The size of the true value type
  static constexpr auto __IL2CPP_VALUE_TYPE_SIZE{ 0x4 };

  /// @brief Field value__, offset: 0x0, size: 0x4, def value: None
  int32_t value__;

  static constexpr bool __IL2CPP_IS_VALUE_TYPE = true;
};
// Non member Declarations
static_assert(offsetof(::OVR::OpenVR::EVRFirmwareError, value__) == 0x0, "Offset mismatch!");

static_assert(sizeof(::OVR::OpenVR::EVRFirmwareError) == 0x4, "Size mismatch!");

} // namespace OVR::OpenVR
