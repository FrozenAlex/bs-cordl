#pragma once
// IWYU pragma private; include "OculusStudios/HierarchyIcons/HierarchyIgnorePrefabOverrides.hpp"
#include "beatsaber-hook/shared/types.hpp"
#include "../../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include "UnityEngine/zzzz__MonoBehaviour_def.hpp"
CORDL_MODULE_EXPORT(HierarchyIgnorePrefabOverrides)
// Forward declare root types
namespace OculusStudios::HierarchyIcons {
class HierarchyIgnorePrefabOverrides;
}
// Write type traits
MARK_REF_T(::OculusStudios::HierarchyIcons::HierarchyIgnorePrefabOverrides*);
DEFINE_IL2CPP_CLASS(::OculusStudios::HierarchyIcons::HierarchyIgnorePrefabOverrides*, "OculusStudios.HierarchyIcons", "HierarchyIgnorePrefabOverrides");
// Dependencies UnityEngine.MonoBehaviour
namespace OculusStudios::HierarchyIcons {
// Is value type: false
// CS Name: OculusStudios.HierarchyIcons.HierarchyIgnorePrefabOverrides
class CORDL_TYPE HierarchyIgnorePrefabOverrides : public ::UnityEngine::MonoBehaviour {
public:
  // Declarations
  static inline ::OculusStudios::HierarchyIcons::HierarchyIgnorePrefabOverrides* New_ctor();

  /// @brief Method .ctor, addr 0x5df3964, size 0x4, virtual false, abstract: false, final false
  inline void _ctor();

protected:
  // Ctor Parameters []
  // @brief default ctor
  constexpr HierarchyIgnorePrefabOverrides();

public:
  // Ctor Parameters [CppParam { name: "", ty: "HierarchyIgnorePrefabOverrides", modifiers: "&&", def_value: None }]
  // @brief delete move ctor to prevent accidental deref moves
  HierarchyIgnorePrefabOverrides(HierarchyIgnorePrefabOverrides&&) = delete;

  // Ctor Parameters [CppParam { name: "", ty: "HierarchyIgnorePrefabOverrides", modifiers: "const&", def_value: None }]
  // @brief delete copy ctor to prevent accidental deref copies
  HierarchyIgnorePrefabOverrides(HierarchyIgnorePrefabOverrides const&) = delete;

  /// @brief IL2CPP Metadata Type Index
  static constexpr uint32_t __IL2CPP_TYPE_DEFINITION_INDEX{ 23368 };

  static constexpr bool __IL2CPP_IS_VALUE_TYPE = false;
};
// Non member Declarations
static_assert(sizeof(::OculusStudios::HierarchyIcons::HierarchyIgnorePrefabOverrides) == 0x20, "Size mismatch!");

} // namespace OculusStudios::HierarchyIcons
