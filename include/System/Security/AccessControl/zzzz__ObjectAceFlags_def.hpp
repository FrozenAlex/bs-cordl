#pragma once
// IWYU pragma private; include "System/Security/AccessControl/ObjectAceFlags.hpp"
#include "beatsaber-hook/shared/types.hpp"
#include "../../../cordl_internals/cordl_internals.hpp"
CORDL_MODULE_INIT
#include <cstddef>
#include <cstdint>
CORDL_MODULE_EXPORT(ObjectAceFlags)
// Forward declare root types
namespace System::Security::AccessControl {
struct ObjectAceFlags;
}
// Write type traits
MARK_VAL_T(::System::Security::AccessControl::ObjectAceFlags);
DEFINE_IL2CPP_CLASS(::System::Security::AccessControl::ObjectAceFlags, "System.Security.AccessControl", "ObjectAceFlags");
// Dependencies
namespace System::Security::AccessControl {
// Is value type: true
// CS Name: System.Security.AccessControl.ObjectAceFlags
struct CORDL_TYPE ObjectAceFlags {
public:
  // Declarations
  using __CORDL_BACKING_ENUM_TYPE = int32_t;

  /// @brief Nested struct __ObjectAceFlags_Unwrapped
  enum struct __ObjectAceFlags_Unwrapped : int32_t {
    __E_None = static_cast<int32_t>(0x0),
    __E_ObjectAceTypePresent = static_cast<int32_t>(0x1),
    __E_InheritedObjectAceTypePresent = static_cast<int32_t>(0x2),
  };

  /// @brief Conversion into unwrapped enum value
  constexpr operator __ObjectAceFlags_Unwrapped() const noexcept {
    return static_cast<__ObjectAceFlags_Unwrapped>(this->value__);
  }

  /// @brief Conversion into unwrapped enum value
  constexpr explicit operator int32_t() const noexcept {
    return static_cast<int32_t>(this->value__);
  }

  // Ctor Parameters []
  // @brief default ctor
  constexpr ObjectAceFlags();

  // Ctor Parameters [CppParam { name: "value__", ty: "int32_t", modifiers: "", def_value: None }]
  constexpr ObjectAceFlags(int32_t value__) noexcept;

  /// @brief Field InheritedObjectAceTypePresent value: I32(2)
  static ::System::Security::AccessControl::ObjectAceFlags const InheritedObjectAceTypePresent;

  /// @brief Field None value: I32(0)
  static ::System::Security::AccessControl::ObjectAceFlags const None;

  /// @brief Field ObjectAceTypePresent value: I32(1)
  static ::System::Security::AccessControl::ObjectAceFlags const ObjectAceTypePresent;

  /// @brief IL2CPP Metadata Type Index
  static constexpr uint32_t __IL2CPP_TYPE_DEFINITION_INDEX{ 3059 };

  /// @brief The size of the true value type
  static constexpr auto __IL2CPP_VALUE_TYPE_SIZE{ 0x4 };

  /// @brief Field value__, offset: 0x0, size: 0x4, def value: None
  int32_t value__;

  static constexpr bool __IL2CPP_IS_VALUE_TYPE = true;
};
// Non member Declarations
static_assert(offsetof(::System::Security::AccessControl::ObjectAceFlags, value__) == 0x0, "Offset mismatch!");

static_assert(sizeof(::System::Security::AccessControl::ObjectAceFlags) == 0x4, "Size mismatch!");

} // namespace System::Security::AccessControl
