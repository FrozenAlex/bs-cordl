...
template <typename K> CORDL_HIDDEN FieldInfo* findField(std::string_view name) {
...
template <typename T, ::i2c::str_lit name, typename K> CORDL_HIDDEN void setStaticField(T&& value) {
...
template <::i2c::type_check::full_class T, ::i2c::str_lit name, typename K> CORDL_HIDDEN void setStaticField(T&& value) {
  static auto field = findField<K>(name.data);
...
/// @tparam name field name
/// @tparam K the class on which the field resides
template <typename T, ::i2c::str_lit name, typename K> [[nodiscard]] CORDL_HIDDEN T getStaticField() {
  static auto field = findField<K>(name.data);
  T val;
...
template <::i2c::type_check::ref_type T, ::i2c::str_lit name, typename K> [[nodiscard]] CORDL_HIDDEN T getStaticField() {
  static auto field = findField<K>(name.data);
  void* val;
